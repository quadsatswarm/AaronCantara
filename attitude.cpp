...
#include <Madgwick.h> 			    	//Angle Filter
...
//Orientation//
float pitch, roll, yaw;     //Angles [deg]
float pitchD, rollD, yawD;  //Angular Rates [deg/s]

//Calibration//
float gyroCal[3] = {-0.404721467, 0.899240366, 0.40121665};  //Gyro Zero Rate Error Correction
float mag_field_strength = 37.88F;                           //Magnetic Field Strength [uT]
float magCal[3] = { -7.98, -18.24, -29.89 };                 //Raw Magnetometer Offsets [uT]
...
void imu(){
  ...
  float x = mag_event.magnetic.x - magCal[0];
  float y = mag_event.magnetic.y - magCal[1];
  float z = mag_event.magnetic.z - magCal[2];
  ...
  rollD = -gyro_event.gyro.x*180.0/PI - gyroCal[0];
  pitchD = gyro_event.gyro.y*180.0/PI - gyroCal[1];
  yawD = gyro_event.gyro.z*180.0/PI - gyroCal[2];
